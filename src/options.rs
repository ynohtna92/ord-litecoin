...
pub struct Options {
  ...
  #[arg(long, help = "Connect to Litecoin Core RPC at <RPC_URL>.")]
...
impl Options {
  ...
  pub(crate) fn bitcoin_rpc_client(&self, wallet: Option<String>) -> Result<Client> {
    ...
    log::info!("Connecting to Litecoin Core at {}", self.rpc_url(None));
...
mod tests {
  ...
  fn use_default_network() {
    ...
    assert_eq!(arguments.options.rpc_url(None), "127.0.0.1:9332/");
  ...
  fn uses_network_defaults() {
    ...
    assert_eq!(arguments.options.rpc_url(None), "127.0.0.1:39332/");
